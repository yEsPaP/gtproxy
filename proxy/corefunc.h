				...
				if (index == 0)
				{
					...
					else if (action == "onShowCaptcha")
					{
						utils::replace(v,
							...
							"numbers|left|\nadd_textbox|",
							"");
						utils::replace(v, "|left|\nadd_text_input|captcha_answer|Answer:||32|\nend_dialog|captcha_submit||Submit|", "");
						auto number1 = v.substr(0, v.find(" +"));
						auto number2 = v.substr(number1.length() + 3, v.length());
//						int result = atoi(number1.c_str()) + atoi(number2.c_str());
//						dbgPrint("Solved captcha as `2" + std::to_string(result) + "``");
